...
def passenger_bus_delay_corr():
    ...
    # new_passenger = new_passenger.iloc[:, 6:]
    
    # print(cols[264:282])
    # print('테스트ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ')
    ...
    #####################################################################
    print('========================쉐이프===========================')
    print('전철 쉐이프', new_passenger.shape)
    ...
    # print('전철', new_passenger.head(3))    
    # print('='*100)
    # print('버스', bus_data.head(3))    
    # print('='*100)
    # print('지연', delay_data.head(3))    
    # print('='*100)
    
    # 전철, 지연시간 합치기
    passenger_delay_data = pd.concat([new_passenger, delay_data], axis=1, ignore_index=True)
    print("passenger_delay_data.shape",passenger_delay_data.shape)
    print('passenger_delay_data.head(10)',passenger_delay_data.head(10))
    
    new_data_ps = pd.DataFrame()
    for label in new_passenger:
        new_data_ps[label] = new_passenger[label]
    for label in delay_data:
        new_data_ps[label] = delay_data[label]
        
    print("============ new data ===========")
    print("head",new_data_ps.head(10))
    print("col ",new_data_ps.columns)
    ...
    new_data_ps_matrix = new_data_ps.corr()
    
    # 상관관계 행렬 출력
    # print("전철 상관관계 상위 5 행 :\n", passenger_matrix.head(8))    
    # print("버스 상관관계 상위 5 행 :\n", bus_matrix.head(8))    
    
    return passenger_matrix, bus_matrix, new_data_ps_matrix



def plot_correlation_heatmap(new_data_ps_matrix, bus_matrix):
    ...
    # 전철 상관관계 히트맵
    plt.subplot(1, 2, 1)
    sns.heatmap(new_data_ps_matrix, annot=True, cmap='coolwarm', fmt=".2f", linewidths=.5)
...
# 데이터 준비 및 상관관계 계산
new_data_ps_matrix, bus_matrix = passenger_bus_delay_corr()


# 시각화
plot_correlation_heatmap(new_data_ps_matrix, bus_matrix)
